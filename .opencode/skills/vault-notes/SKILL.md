...
Six top-level buckets (display order maintained by an Obsidian plugin, no
numbering):
...
to be. Treat Inbox contents as immutable source material: process + discard
original, never edit in place. **Processing an Inbox item means ingesting it
(e.g. into the wiki) and then discarding the original — never move it to
`Archief/`.** Archief is for inactive PARA items, not for processed source
material.
...
1. Determine the bucket (decision tests above) and the note type (person,
   klant, process, `_home.md` MOC, or Dagboek entry).
2. Pick the path per the naming conventions (date-prefix, type-prefix, or
   kebab-case slug).
3. Write the file using the matching template below.
4. Link it from related notes with `[[wikilinks]]` (people, processes,
   projects).
5. Check Obsidian's **Unlinked Mentions** for missed links and offer to fix
   them.

## Edge cases

- **Ambiguous bucket** — a note that could be Project or Area: ask the user
  whether it has an end date rather than guessing.
- **Depth 4** — if a path would nest four levels deep, flatten it (never
  level 4).
- **Fewer than 3 files** — don't create a folder for a topic; use a `_home.md`
  MOC that links the files instead.
- **Inbox item** — process (ingest) then **discard the original**; never move
  it to `Archief/`.
- **Eigen project** — when the user is the client, write `**Klant:** eigen
  project` and create no client entity note.

## Depth rules

- Level 1: the six buckets.
...
### `klant - naam.md` (client entity note — a hub others link to)

Clients are **metadata, not containers**: projects live at
`Projecten/<project>/` (Level 2) and the client note lives in
`Aandachtsgebieden/Werk/Klanten/`. Each project `_home.md` carries a
`**Klant:** [[klant]]` line. Eigen projecten (user is the client) get
`**Klant:** eigen project` and no entity note.
...
status, and links to everything else. It is the folder's `index.md`. For work
projects, include a `**Klant:** [[client]]` line (or `eigen project`).
